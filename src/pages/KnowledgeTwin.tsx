...
import { PreventiveMaintenancePanel } from "@/components/features/analytics/PreventiveMaintenancePanel";
import { knowledgeTwinDemoAssets } from "@/lib/demoScenarios";
import type {
...
export default function KnowledgeTwin() {
  const [activeSection, setActiveSection] = useState<"twin" | "maintenance">("twin");
  ...
  const fetchBundledFile = async (path: string, filename: string) => {
  ...
  const loadBundledMaintenanceDemo = async () => {
    setLoading(true);
    try {
      const [logsFile, incidentsFile, sopDemoFile] = await Promise.all([
        fetchBundledFile(knowledgeTwinDemoAssets.logs.path, knowledgeTwinDemoAssets.logs.label),
        fetchBundledFile(knowledgeTwinDemoAssets.incidents.path, knowledgeTwinDemoAssets.incidents.label),
        fetchBundledFile(knowledgeTwinDemoAssets.sop.path, knowledgeTwinDemoAssets.sop.label),
      ]);
      const [logsRes, incidentsRes, sopRes] = await Promise.all([
        engineeringApi.uploadKnowledgeLogs(logsFile, false),
        engineeringApi.uploadKnowledgeIncidents(incidentsFile, false),
        engineeringApi.uploadKnowledgeSop(sopDemoFile, "Motor Maintenance SOP", "motor", false),
      ]);
      setLogFile(logsFile);
      setIncidentFile(incidentsFile);
      setSopFile(sopDemoFile);
      setLogsUploaded(true);
      setIncidentsUploaded(true);
      setSopUploaded(true);
      setSopName("Motor Maintenance SOP");
      setSopAssetType("motor");
      const ids = [logsRes.dataset_id, incidentsRes.dataset_id, sopRes.dataset_id]
        .filter(Boolean)
        .map((value) => String(value));
      if (ids.length) {
        setPendingDatasetIds((prev) => Array.from(new Set([...prev, ...ids])));
      }
      toast.success("Bundled maintenance demo loaded.");
    } catch (error) {
      toast.error(getErrorMessage(error, "Failed to load the bundled maintenance demo."));
    } finally {
      setLoading(false);
    }
  };
  ...
  const riskLabel = recommendation?.risk?.risk_level || "Not assessed";
  const urgencyLabel = confidencePct >= 80 ? "Escalate now" : confidencePct >= 55 ? "Review this shift" : "Monitor";
  const riskValue = recommendation?.risk?.risk_score || 0;
  const failureWindow = riskValue >= 0.8 ? "0-7 days" : riskValue >= 0.55 ? "1-4 weeks" : "Unknown";
  const downtimeRisk = riskValue >= 0.8 ? "High" : riskValue >= 0.55 ? "Moderate" : "Low";
  const nextStep = chosenAction !== "N/A" ? chosenAction : recommendation?.ai_summary || "Load a twin and run a deterministic query.";
  ...
  return (
    <main className="min-h-screen bg-slate-50 py-6">
      <section className="container px-4 space-y-4">
        ...
        <Tabs value={activeSection} onValueChange={(value) => setActiveSection(value as "twin" | "maintenance")} className="space-y-4">
          <TabsList className="grid w-full grid-cols-1 sm:w-auto sm:grid-cols-2">
            <TabsTrigger value="twin">Knowledge Twin</TabsTrigger>
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-xl border border-red-200 bg-red-50 p-4">
            <div className="text-xs uppercase tracking-wide text-red-700">Risk</div>
            <div className="mt-2 text-lg font-semibold text-red-900">{riskLabel}</div>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            <div className="text-xs uppercase tracking-wide text-slate-500">Urgency</div>
            <div className="mt-2 text-lg font-semibold text-slate-900">{urgencyLabel}</div>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            <div className="text-xs uppercase tracking-wide text-slate-500">Failure Window</div>
            <div className="mt-2 text-lg font-semibold text-slate-900">{failureWindow}</div>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            <div className="text-xs uppercase tracking-wide text-slate-500">Downtime Risk</div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            ...
            <div className="mt-2 text-lg font-semibold text-slate-900">{evidenceCount}</div>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            <div className="text-xs uppercase tracking-wide text-slate-500">Confidence</div>
            <div className="mt-2 text-lg font-semibold text-slate-900">{confidencePct}%</div>
          </div>
          <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 md:col-span-2">
            <div className="text-xs uppercase tracking-wide text-emerald-700">Next Step</div>
            <div className="mt-2 text-sm font-medium leading-6 text-emerald-900">{nextStep}</div>
          </div>
        </div>
        ...
        <Card className="border-slate-200">
          <CardHeader>
            ...
            <CardDescription>Use this sequence for the preventive-maintenance walkthrough.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-3 md:grid-cols-4 text-sm">
            <div className="rounded-md border bg-slate-50 p-3">
              <div className="text-xs uppercase tracking-wide text-slate-500">1. Load bundled assets</div>
              <div className="mt-2">Start with the packaged logs, incidents, and SOP so the evidence graph is ready.</div>
            </div>
            <div className="rounded-md border bg-slate-50 p-3">
              <div className="text-xs uppercase tracking-wide text-slate-500">2. Load the twin</div>
              <div className="mt-2">Use the selected asset and optional failure mode to build the graph and risk view.</div>
            </div>
            <div className="rounded-md border bg-slate-50 p-3">
              <div className="text-xs uppercase tracking-wide text-slate-500">3. Run a deterministic query</div>
              <div className="mt-2">Ask what failed, what worked historically, and what should happen next.</div>
            </div>
            <div className="rounded-md border border-emerald-200 bg-emerald-50 p-3">
              <div className="text-xs uppercase tracking-wide text-emerald-700">4. Escalate to action</div>
              <div className="mt-2">Use risk, urgency, confidence, and next step as the maintenance handoff to operations.</div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-slate-200">
          <CardHeader>
            <CardTitle>Bundled Maintenance Demo</CardTitle>
            <CardDescription>Load the packaged logs, incidents, and SOP assets for a ready-made preventive-maintenance walkthrough.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-3 md:grid-cols-3">
              <div className="rounded-md border border-slate-200 bg-white p-3 text-sm text-slate-700">
                <div className="text-xs uppercase tracking-wide text-slate-500">Logs</div>
                <div className="mt-2">{knowledgeTwinDemoAssets.logs.label}</div>
              </div>
              <div className="rounded-md border border-slate-200 bg-white p-3 text-sm text-slate-700">
                <div className="text-xs uppercase tracking-wide text-slate-500">Incidents</div>
                <div className="mt-2">{knowledgeTwinDemoAssets.incidents.label}</div>
              </div>
              <div className="rounded-md border border-slate-200 bg-white p-3 text-sm text-slate-700">
                <div className="text-xs uppercase tracking-wide text-slate-500">SOP</div>
                <div className="mt-2">{knowledgeTwinDemoAssets.sop.label}</div>
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button onClick={loadBundledMaintenanceDemo} disabled={loading}>
                {loading ? "Loading Demo..." : "Load Bundled Maintenance Demo"}
              </Button>
              <Button variant="outline" onClick={() => setActiveSection("maintenance")}>
                Open Preventive Maintenance View
              </Button>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-slate-200">
          ...
          <CardContent>
            <PersistedResizableGroup
              storageKey="workspace.knowledgeTwin.twin.v1"
            ...
            >
              ...
              <ResizablePanel defaultSize={58} minSize={24}>
                <div className="h-full pl-0 lg:pl-4">
        ...
        <Card className="border-slate-200">
          ...
          <CardContent>
            <PersistedResizableGroup
              storageKey="workspace.knowledgeTwin.ai.v1"
              direction="horizontal"
              defaultSizes={[45, 55]}
              minSizes={[24, 24]}
              enabled={isDesktopLayout}
            ...
            >
              <ResizablePanel defaultSize={45} minSize={24}>
                <div className="h-full pr-0 lg:pr-4">
                  <div className="rounded-md border border-slate-200 bg-white p-4 h-full">
        ...
        <Card className="border-slate-200">
          ...
          <CardContent>
            <PersistedResizableGroup
              storageKey="workspace.knowledgeTwin.python.v1"
              direction="vertical"
              defaultSizes={[45, 55]}
              minSizes={[20, 20]}
              enabled={isDesktopLayout}
            ...
            >
              <ResizablePanel defaultSize={45} minSize={20}>
                <div className="h-full overflow-auto space-y-3 pb-3">
                  <Textarea
                    rows={10}
                    value={pythonCode}
              ...
              <ResizablePanel defaultSize={55} minSize={20}>
                ...
                {(pythonResult.images || []).length > 0 && (
                  ...
                  {!pythonResult && (
                    <div className="text-sm text-slate-500">No Python output yet.</div>
                  )}
                </div>
              </ResizablePanel>
            </PersistedResizableGroup>
