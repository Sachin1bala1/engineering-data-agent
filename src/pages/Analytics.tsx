...
const Analytics: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'enterprise'|'chat'|'data'|'highsize'|'slides'|'compare'|'rootcause'|'analyzer'|'doe'>('chat');
  ...
  const [mainPaneRatio, setMainPaneRatio] = useState<number>(() => {
    ...
    return Number.isFinite(parsed) ? Math.max(20, Math.min(90, parsed)) : 68;
  });
  ...
  useEffect(() => {
    ...
    const supported = new Set(['enterprise', 'chat', 'data', 'highsize', 'slides', 'compare', 'rootcause', 'analyzer', 'doe']);
        ...
        <div
          ...
          className={`grid grid-cols-1 ${
            sidePanelCollapsed
              ...
              : isDesktopLayout
                ? "lg:grid-cols-[minmax(0,1fr)_8px_minmax(220px,1fr)] gap-0"
                : "lg:grid-cols-3 gap-6"
          }`}
          style={
            !sidePanelCollapsed && isDesktopLayout
              ? { gridTemplateColumns: `minmax(0, ${mainPaneRatio}fr) 8px minmax(220px, ${100 - mainPaneRatio}fr)` }
              : undefined
          }
        >
          <div className="min-w-0">
            <Tabs value={activeTab} onValueChange={(v:any)=>setActiveTab(v)} className="h-full">
              <TabsList className={`${workspaceTabListClassName} grid-cols-2 sm:grid-cols-4 md:grid-cols-6 xl:grid-cols-9`}>
          ...
          <div className={`self-start ${isDesktopLayout ? "lg:pl-4 lg:sticky lg:top-24" : ""}`}>
            ...
            >
              <div className="pb-3 text-sm text-muted-foreground">Ask for deterministic findings, data edits, what-if, charts, and auto-run Python when needed.</div>
                <ResizablePanelGroup direction="vertical" className="h-[68vh] min-h-[520px]">
                  <ResizablePanel defaultSize={28} minSize={18}>
                    <div className="h-full p-1 space-y-2 overflow-auto">
                      ...
                      />
                      <Button
                        className={`w-full ${workspaceToolbarPrimaryButtonClassName}`}
                  ...
                  <ResizablePanel defaultSize={24} minSize={12}>
                    <div className="h-full rounded-md border p-2 overflow-y-auto space-y-2">
                      {messages
                        .filter((m) => m.role === "ai")
                        .slice(-8)
                        .map((m) => (
                  ...
                  <ResizablePanel defaultSize={48} minSize={20}>
                    <div className="h-full space-y-2 rounded-md border p-2 overflow-auto">
                      ...
                      {!((isHighSizeContext ? highRunnerCode : slideRunnerCode)
                        || (isHighSizeContext ? highRunnerOutput : slideRunnerOutput)
                        || (isHighSizeContext ? highRunnerChart : slideRunnerChart)) && (
                        <div className="text-xs text-muted-foreground">No Python runner output yet.</div>
                      )}
              ...
              <ResizableHandle withHandle />
              <ResizablePanel defaultSize={42} minSize={16}>
                <div className="h-full pl-1">
                  <ResizablePanelGroup direction="vertical" className="h-full min-h-[320px]">
                    <ResizablePanel defaultSize={42} minSize={18}>
                      <div className="h-full overflow-auto pb-3">
                        <WorkspaceResultCard title="Slide Builder" className="h-full" contentClassName="space-y-3">
                            <div className="text-sm text-muted-foreground">Manual slide title/findings editor. Expand only when needed.</div>
                            <Collapsible open={slideBuilderOpen} onOpenChange={setSlideBuilderOpen}>
                              <CollapsibleTrigger asChild>
                                <Button variant="outline" className={`w-full justify-between ${workspaceToolbarButtonClassName}`}>
                    ...
                    <ResizablePanel defaultSize={58} minSize={18}>
                      <div className="h-full overflow-auto pt-3">
                        <WorkspaceResultCard title="Data Summary" className="h-full" contentClassName="space-y-4">
