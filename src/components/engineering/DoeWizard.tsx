...
export function DoeWizard() {
  ...
  const selectedMethodLabel = METHODS.find((method) => method.id === selectedMethod)?.label || recommendation?.method || "Not selected";
    ...
    <div className="space-y-6">
      ...
      <Card className="border-primary/15">
        <CardHeader>
          ...
          <CardDescription>Keep the business goal, factor scope, and run budget visible while you build the design.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-5">
          <div className="rounded-md border bg-muted/20 p-3">
            <div className="text-xs uppercase tracking-wide text-muted-foreground">Goal</div>
            <div className="mt-2 text-sm font-medium">{GOALS.find((item) => item.id === goal)?.label || goal}</div>
          </div>
          <div className="rounded-md border bg-muted/20 p-3">
            <div className="text-xs uppercase tracking-wide text-muted-foreground">Factors</div>
            <div className="mt-2 text-sm font-medium">{factorCount}</div>
          </div>
          <div className="rounded-md border bg-muted/20 p-3">
            <div className="text-xs uppercase tracking-wide text-muted-foreground">Run Budget</div>
            <div className="mt-2 text-sm font-medium">{runBudget} runs</div>
          </div>
          <div className="rounded-md border bg-muted/20 p-3">
            <div className="text-xs uppercase tracking-wide text-muted-foreground">Recommended Method</div>
            <div className="mt-2 text-sm font-medium">{selectedMethodLabel}</div>
          </div>
          <div className="rounded-md border border-emerald-200 bg-emerald-50 p-3">
            <div className="text-xs uppercase tracking-wide text-emerald-700">Estimated Runs</div>
            <div className="mt-2 text-sm font-medium">{estimatedRuns ?? matrix?.matrix?.length ?? "Pending design"}</div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Follow these steps for the manufacturing DOE story.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-4 text-sm">
          <div className="rounded-md border bg-muted/20 p-3">
            <div className="text-xs uppercase tracking-wide text-muted-foreground">1. Define the outcome</div>
            <div className="mt-2">Choose yield, defects, or optimization before touching factor settings.</div>
          </div>
          <div className="rounded-md border bg-muted/20 p-3">
            <div className="text-xs uppercase tracking-wide text-muted-foreground">2. Set factors and budget</div>
            <div className="mt-2">Use a realistic run budget so the recommendation matches plant constraints.</div>
          </div>
          <div className="rounded-md border bg-muted/20 p-3">
            <div className="text-xs uppercase tracking-wide text-muted-foreground">3. Generate the design</div>
            <div className="mt-2">Review the recommended method and estimated runs before exporting the matrix.</div>
          </div>
          <div className="rounded-md border border-emerald-200 bg-emerald-50 p-3">
            <div className="text-xs uppercase tracking-wide text-emerald-700">4. Explain the decision</div>
            <div className="mt-2">Use the results summary to justify the operating window, then open advanced analysis if challenged.</div>
          </div>
        </CardContent>
      </Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="rounded-md border bg-muted/20 px-4 py-3">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <div className="text-sm font-medium">Advanced analysis</div>
                  <div className="text-xs text-muted-foreground">
                    Correlation, regression diagnostics, residual checks, and ANOVA are the technical layer behind this recommendation.
                  </div>
                </div>
                <Button variant="outline" onClick={() => setShowAdvancedAnalysis((current) => !current)}>
                  {showAdvancedAnalysis ? "Hide Advanced Focus" : "Highlight Advanced Focus"}
                </Button>
              </div>
            </div>

            <div className={`space-y-2 ${showAdvancedAnalysis ? "ring-1 ring-primary/20 rounded-lg p-3" : ""}`}>
