...
interface ComparisonCopilotPanelProps {
...
export function ComparisonCopilotPanel({ report }: ComparisonCopilotPanelProps) {
  ...
  const [contextInfo, setContextInfo] = useState<ComparisonCopilotContextResponse | null>(null);
  const [scriptDraft, setScriptDraft] = useState("");
  const [runResult, setRunResult] = useState<ScriptRunResponse | null>(null);
  const [isRunningScript, setIsRunningScript] = useState(false);
  const [sessionId] = useState(() => {
    const existing = localStorage.getItem("compare_copilot_session_id");
    if (existing) return existing;
    const created = `compare_session_${Date.now()}`;
    localStorage.setItem("compare_copilot_session_id", created);
    return created;
  });
  ...
  useEffect(() => {
    const loadStatus = async () => {
      try {
        const result = await engineeringApi.getComparisonCopilotStatus();
        setStatus(result);
      } catch (err: unknown) {
        console.error("Comparison copilot status error:", err);
      }
    };
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      ...
      try {
        const result = await engineeringApi.getComparisonCopilotContext(report.report_id, sessionId);
        setContextInfo(result);
      } catch (err: unknown) {
        setError(getErrorMessage(err, "Failed to load comparison context"));
      } finally {
  ...
  const handleRefreshModel = async () => {
    ...
    try {
      const result = await engineeringApi.refreshComparisonCopilotModel();
      setStatus((prev) => ({
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      const result = await engineeringApi.queryComparisonCopilot({
      ...
      setError(getErrorMessage(err, "Comparison copilot request failed"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleResetSession = async () => {
    setError(null);
    try {
      await engineeringApi.resetComparisonCopilotSession(sessionId);
      setMessages([]);
      setResponse(null);
      setScriptDraft("");
      setRunResult(null);
  ...
  return (
    ...
    >
        <PersistedResizableGroup
          storageKey="workspace.compare.copilot.v1"
          direction="vertical"
          ...
          <ResizablePanel defaultSize={22} minSize={18}>
            <div className="h-full overflow-auto space-y-4 pr-1">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <WorkspaceMetricChip label="Signals" value={report.signal_comparison.length} />
              ...
              <div className="flex flex-col gap-3">
                <select
                  ...
                  className={`border border-gray-300 bg-white ${workspaceToolbarSelectClassName}`}
                >
                  {roles.map((option) => (
                ...
                <input
                  ...
                  placeholder="Ask about the comparison: e.g., Which signal drives the deviation?"
          ...
          <ResizablePanel defaultSize={40} minSize={20}>
            <WorkspaceResultCard title="Copilot Summary" className="h-full" contentClassName="space-y-3">
              {response ? (
                ...
                <div className="text-sm text-gray-500">Ask a question to generate comparison guidance and optional Python code.</div>
              )}
            </WorkspaceResultCard>
          </ResizablePanel>
          <ResizableHandle withHandle />
              ...
              {contextInfo && (
                <details className="text-xs text-gray-500">
                  ...
                  <pre className="mt-2 bg-gray-50 border rounded p-3 whitespace-pre-wrap">
                    {JSON.stringify(contextInfo.context, null, 2)}
                  </pre>
                </details>
              )}
              {messages.length > 0 && (
