"""
Electric motor shaft misalignment detection.
"""
...
def _detect(context) -> DetectionResult:
    ...
    indicators = [
        ...
        build_indicator("temperature_trend", temp_trend,
                        ...
                        "Temperature trend tracking vibration", 72),
    ...
    early = vib_z >= 1.8 and vib_trend > 0.01
    mid = vib_z >= 2.5 and vib_persist >= 0.5 and temp_z >= 0.5
    late = vib_z >= 3.0 and temp_z >= 1.5 and vib_persist >= 0.6
    ...
    explanation = (
        "Aligned rise in vibration and temperature with sustained persistence "
        "indicates shaft misalignment or coupling offset."
    )
...
def build_definition() -> FailureModeDefinition:
    return FailureModeDefinition(
        failure_mode_id="motor_shaft_misalignment",
        asset_type=AssetType.ELECTRIC_MOTOR,
        physical_root_cause="Coupling offset or angular misalignment",
        observable_indicators=[
            "Moderate vibration elevation",
            "Vibration trend increase over 72 hours",
            "Temperature trend following vibration",
            "Persistent elevation over 24 hours"
        ],
        early_indicators=[
            "Moderate vibration increase with positive trend",
            "Temperature trend begins to follow vibration"
        ],
        mid_stage_indicators=[
            "Sustained vibration elevation over 24 hours",
            "Temperature deviation becomes noticeable"
        ],
        late_stage_indicators=[
            "High vibration with persistent temperature rise",
            "Potential coupling wear or baseplate movement"
        ],
        severity_score=0.7,
        risk_severity_impact="Moderate to high impact with accelerated bearing wear risk",
        recommended_action=(
            "Inspect coupling alignment and check for soft foot. "
            "Realign shaft and verify baseplate condition."
        ),
        urgency_days=14,
        skill_level_required=SkillLevel.MAINTENANCE_TECHNICIAN,
        detection_logic_text=(
            "Vibration z-score >= 1.8 with persistent trend and temperature trend "
            "tracking vibration over 24-72 hours."
        ),
        detection_logic=_detect,
        confidence_calculation=(
            "Indicator coverage weighted by persistence over 24 hours."
        )
    )
