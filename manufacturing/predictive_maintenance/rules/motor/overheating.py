"""
Electric motor thermal overload detection.
"""
...
def _detect(context) -> DetectionResult:
    ...
    temp_persist = persistence_ratio(temp_values_4h, threshold=85.0)
    ...
    explanation = (
        "Sustained high temperature with rapid upward trend indicates thermal overload "
        "and insulation stress."
    )

    detection_logic = (
        "Trigger when temperature z-score is high with sustained 4h persistence "
...
def build_definition() -> FailureModeDefinition:
    return FailureModeDefinition(
        failure_mode_id="motor_thermal_overload",
        asset_type=AssetType.ELECTRIC_MOTOR,
        physical_root_cause="Thermal overload leading to insulation degradation",
        observable_indicators=[
            "High temperature deviation",
            "Rapid temperature trend over 24 hours",
            "Persistence above 85 degC over 4 hours"
        ],
        early_indicators=[
            "Temperature trend acceleration",
            "Temperature z-score elevated with short persistence"
        ],
        mid_stage_indicators=[
            "Sustained temperature above 85 degC",
            "Rapid temperature increase over 24 hours"
        ],
        late_stage_indicators=[
            "Critical temperature deviation with sustained persistence",
            "High risk of insulation damage"
        ],
        severity_score=0.9,
        risk_severity_impact="Critical impact with immediate failure risk",
        recommended_action=(
            "Verify loading, cooling airflow, and insulation condition. "
            "Reduce load or schedule immediate inspection."
        ),
        urgency_days=3,
        skill_level_required=SkillLevel.RELIABILITY_ENGINEER,
        detection_logic_text=(
            "Temperature z-score >= 2.0 with sustained persistence above 85 degC "
            "and rapid temperature trend."
        ),
        detection_logic=_detect,
        confidence_calculation=(
            "Indicator coverage weighted by sustained temperature persistence."
        )
    )
