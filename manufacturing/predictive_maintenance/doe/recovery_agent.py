...
def suggest_fix(error_message: str, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    instructions = (
        "You are the DOE Error Recovery Agent. Read the error and propose fixes. "
        "Do NOT auto-correct. Return JSON only with keys: issue, suggestions, notes."
    )
    payload = {
        "error": error_message,
        "context": context,
    ...
    try:
        ...
        return {
            "issue": parsed.get("issue", error_message),
