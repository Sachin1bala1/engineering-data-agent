...
def validate(plan: Dict[str, Any], results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    instructions = (
        "You are the DOE Validation Agent. Verify tests are valid, sample sizes adequate, "
        "results internally consistent. Do NOT compute numbers. "
        "Return JSON only with keys: status, issues, recommendations."
    )
    payload = {
        "plan": plan,
    ...
    try:
        ...
        if not parsed:
            return {"status": "PASS", "issues": ["Validation response could not be parsed."], "recommendations": []}
        return {
        ...
        return {"status": "PASS", "issues": ["Validation unavailable; deterministic pass."], "recommendations": []}
