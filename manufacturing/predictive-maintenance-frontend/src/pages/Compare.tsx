...
import { ComparisonCopilotPanel } from '../components/ComparisonCopilotPanel';
import { apiClient, getErrorMessage } from '../lib/api';
import type { ComparisonReport, AgentLogsResponse } from '../types/api';
import {
  LineChart,
  Line,
  XAxis,
...
export function Compare() {
  ...
  const handleRun = async () => {
    ...
    setIsRunning(true);
    try {
      const result = await apiClient.uploadComparison(
        baselineFile,
        experimentFile,
        assetId || undefined,
        analysisMode
      ...
      const reportData = await apiClient.getComparisonReport(result.report_id);
      setReport(reportData);
      const logData = await apiClient.getComparisonAgentLogs(result.report_id);
      setLogs(logData);
    } catch (err: unknown) {
                  ...
                  <tbody>
                    {report.signal_comparison.map((signal) => (
                      <tr key={signal.signal_name} className="border-t">
                        ...
                        <td className="p-2">
                          {signal.deviation_metrics.percent_delta?.toFixed(1) ?? 'N/A'}%
                        </td>
                        <td className="p-2">
                          {signal.deviation_metrics.z_score_vs_baseline?.toFixed(2) ?? 'N/A'}
                        </td>
