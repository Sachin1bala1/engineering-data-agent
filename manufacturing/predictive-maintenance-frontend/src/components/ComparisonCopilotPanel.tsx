...
export function ComparisonCopilotPanel({ report }: ComparisonCopilotPanelProps) {
  ...
  useEffect(() => {
    const loadStatus = async () => {
      try {
        const result = await apiClient.getComparisonCopilotStatus();
        setStatus(result);
      } catch (err: unknown) {
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      ...
      try {
        const result = await apiClient.getComparisonCopilotContext(report.report_id, sessionId);
  ...
  const handleRefreshModel = async () => {
    ...
    try {
      const result = await apiClient.refreshComparisonCopilotModel();
      setStatus((prev) => ({
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      const result = await apiClient.queryComparisonCopilot({
  ...
  const handleResetSession = async () => {
    setError(null);
    try {
      await apiClient.resetComparisonCopilotSession(sessionId);
      setMessages([]);
      setResponse(null);
      setScriptDraft('');
      setRunResult(null);
