    ...
    <div className="space-y-3">
      <Card>
        <CardContent className="pt-4 flex flex-wrap items-center gap-2">
          <Button type="button" variant="outline">Dataset: {datasetName}</Button>
          <Button type="button" variant="outline" onClick={() => saveConfig()}>Export</Button>
          <Button type="button" variant="outline" onClick={saveConfig}>Save Graph</Button>
          <Button type="button" variant="outline">AI Insights</Button>
          <Button type="button" variant="outline" onClick={resetExploration}>Reset</Button>
          <Button type="button" variant="outline">Settings</Button>
          <div className="text-xs text-muted-foreground ml-auto">Rows: {filteredRows.length.toLocaleString()} / {rows.length.toLocaleString()}</div>
        </CardContent>
      </Card>

      <div className="grid grid-cols-1 xl:grid-cols-[290px_1fr_320px] gap-3">
        <div className="space-y-3">
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-base">Columns</CardTitle></CardHeader>
            <CardContent><VariableList variables={variables} /></CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-base">Graph Roles</CardTitle></CardHeader>
        ...
        <div className="space-y-3 min-w-0">
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-base">Graph Canvas</CardTitle></CardHeader>
            <CardContent>
                ...
                <div className="flex flex-wrap gap-2">
                  {layers.map((layer) => (
                    <button
                      ...
                      type="button"
                      draggable
                      className="h-8 px-2 rounded border bg-background text-xs inline-flex items-center gap-1"
                      ...
                      .map((layer) => (
                        <div key={`active:${layer.type}`} className="inline-flex items-center gap-1">
                          <button
                            type="button"
                            draggable
                            className={`px-2 py-0.5 rounded border text-xs ${selectedLayerType === layer.type ? "bg-primary text-primary-foreground" : ""}`}
                          ...
                          <button type="button" className="text-[10px] px-1 border rounded" title="Move up" onClick={() => moveLayer(layer.type, -1)}>^</button>
                          <button type="button" className="text-[10px] px-1 border rounded" title="Move down" onClick={() => moveLayer(layer.type, 1)}>v</button>
                        </div>
                      ))
                  )}
                </div>
        ...
        <div className="space-y-3">
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-base">Chart Elements</CardTitle></CardHeader>
              ...
              <div className="grid grid-cols-2 gap-2">
                {layers.map((layer) => (
                  <button
                    key={layer.id}
                    type="button"
                    className={`h-8 rounded border px-2 text-xs text-left inline-flex items-center gap-1 ${layer.enabled ? "bg-primary text-primary-foreground" : ""}`}
                    onClick={() => {
              ...
              {selectedLayer && (
                <div className="rounded border p-2 space-y-2">
                  ...
                  <div>
                    ...
                    >
                      <SelectTrigger><SelectValue placeholder="Legend behavior" /></SelectTrigger>
                      <SelectContent>
          ...
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-base">Global Statistics Defaults</CardTitle></CardHeader>
          ...
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-base">Axis Controls</CardTitle></CardHeader>
            <CardContent className="space-y-2 text-sm">
              <div className="grid grid-cols-2 gap-2">
                <Select value={axisX.scale} onValueChange={(v) => setAxisX({ ...axisX, scale: v as any })}>
                  <SelectTrigger><SelectValue placeholder="X scale" /></SelectTrigger>
                  <SelectContent>
                ...
                <Select value={axisY.scale} onValueChange={(v) => setAxisY({ ...axisY, scale: v as any })}>
                  <SelectTrigger><SelectValue placeholder="Y scale" /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="grid grid-cols-2 gap-2">
                <Input type="number" placeholder="X ref line" value={axisX.referenceLine ?? ""} onChange={(e) => setAxisX({ ...axisX, referenceLine: e.target.value === "" ? undefined : Number(e.target.value) })} />
                <Input type="number" placeholder="Y ref line" value={axisY.referenceLine ?? ""} onChange={(e) => setAxisY({ ...axisY, referenceLine: e.target.value === "" ? undefined : Number(e.target.value) })} />
              </div>
            </CardContent>
          </Card>
                    ...
                    <div key={f.column}>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {f.selectedValues.map((v) => (
                          <button key={`${f.column}:${v}`} type="button" className="px-2 py-0.5 rounded border" onClick={() => removeFilterValue(f.column, v)}>
                            {v || "(blank)"} x
                          </button>
                        ))}
                      </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              <Input value={config.title} onChange={(e) => patchConfig((p) => ({ ...p, title: e.target.value }))} placeholder="Chart title" />
              <Input value={config.xLabel} onChange={(e) => patchConfig((p) => ({ ...p, xLabel: e.target.value }))} placeholder="X label" />
              <Input value={config.yLabel} onChange={(e) => patchConfig((p) => ({ ...p, yLabel: e.target.value }))} placeholder="Y label" />
              ...
              <div className="rounded border p-2 space-y-2">
                ...
                <div className="flex gap-2">
                  <Button type="button" variant="outline" onClick={saveTemplate}>Save Template</Button>
                  <Button type="button" variant="outline" onClick={loadTemplate}>Load Template</Button>
                  <Button type="button" variant="outline" onClick={deleteTemplate}>Delete</Button>
                </div>
              </div>
              <div className="flex gap-2">
                <Button type="button" variant="outline" onClick={saveConfig}>Save Config</Button>
                <label className="h-9 px-3 rounded-md border text-sm inline-flex items-center cursor-pointer">
                  Load Config
